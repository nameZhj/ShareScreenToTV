...
### 4. 🎮 专为电视遥控器定制的交互与焦点系统
- **菜单键唤醒**：在投屏过程中，随时按下遥控器的 **OK 键**、**设置键 (Settings)**、**菜单键 (Menu)** 或书签键，即可呼出管理菜单。
- **高对比度焦点样式与动效**：所有按钮、卡片、弹窗选项均具备高对比度亮蓝/亮红外发光边框、Z 轴立体浮起（`translationZ = 12f`）与防裁切保护（`clipChildren=false`）。

### 5. 📐 智能画面缩放与比例保持 (Aspect Ratio Fit)
- 电视端提供两种画面展示模式：
  - **默认模式**：满屏铺满当前画面。
  - **平铺模式 (Aspect Fit)**：实时感知手机画面宽高比，保持原有比例不拉伸变形，并使最长边铺满屏幕（自动适应竖屏/横屏并居中显示）。

### 6. 🔄 网络变化自适应与断连自动返回
- **动态 IP 感知**：电视端实时监听 Wi-Fi / 以太网状态，网络切换或 DHCP 重新分配 IP 时自动刷新界面并重新生成连接二维码。
- **断连平滑重置**：当电视端关闭或网络中断时，手机端底层长连接自动响应，安全注销前台投屏服务并平滑切回扫码连接页面。
